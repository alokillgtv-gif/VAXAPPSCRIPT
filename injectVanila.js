// ============================================================
// Web Interactive Lab & Inspector Dashboard PRO v16.6
// STANDALONE VERSION - Không cần Tampermonkey / Greasemonkey
// ============================================================
// Hướng dẫn sử dụng:
//   1. Mở trang web bất kỳ, nhấn F12 → Console.
//   2. Paste toàn bộ nội dung file này vào Console và nhấn Enter.
//   3. Hoặc lưu thành Bookmarklet (thêm javascript: ở đầu, xóa xuống dòng).
// ============================================================
...
(function() {
    ...
    // --------------------------------------------------------
    // 2. DANH SÁCH JS CẦN LOAD (theo đúng thứ tự @require)
    // --------------------------------------------------------
    const jsUrls = [
    ...
    // --------------------------------------------------------
    async function boot() {
        console.log('[WebLab Standalone] Đang tải thư viện...');

        // Load CSS song song
        await Promise.all(cssUrls.map(u => loadCSS(u)));

        // Load JS tuần tự (đảm bảo dependency đúng thứ tự)
        for (const src of jsUrls) {
            await loadScript(src);
        }

        console.log('[WebLab Standalone] Thư viện đã sẵn sàng. Khởi chạy Lab...');

        // Hàm chính (tách biệt để xử lý cả trường hợp đã loaded)
        function runLab() {
            setTimeout(() => {
...
`;
            ...
            /* === Begin Module IIFE & Event Listener Cleanup: 5318 === */
                ...
                /* === End Module IIFE & Event Listener Cleanup === */
            }, 10);
        }
        ...
        if (document.readyState === 'complete') {
            ...
            window.addEventListener('load', runLab);
        }
    }

    boot().catch(err => console.error('[WebLab Standalone] Lỗi khởi động:', err));
})();
