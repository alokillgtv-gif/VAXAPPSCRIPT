...
.lab-error-line-link: hover { color: #fff!important;text-decoration: none!important; }
        ...
        // 8. BỘ MÁY THỰC THI JAVASCRIPT ĐỘNG (FIXED)
        // ==========================================
        /**
 * Enhanced JS Lab Engine v2
 * Cơ chế báo lỗi thông minh: Dịch + Gợi ý fix + Cải thiện stack trace
 */

    /**
 * Enhanced JS Lab Engine v4 - Robust Error Reporting
 * Cải tiến chính:
 * 1. Prefix 1 dòng duy nhất → offset luôn = 1 (không cần đếm)
 * 2. Không dùng window.onerror (tránh xung đột) → dùng addEventListener('error') + postMessage
 * 3. Mỗi lần chạy có sourceURL duy nhất → lọc chính xác lỗi thuộc về mình
 * 4. Click vào log → scroll + highlight CodeMirror
 */
    ...
    function scrollToLine(line, col) {
        if (window.__labJsEditor && typeof window.__labJsEditor.setCursor === 'function') {
            ...
            // Highlight nền dòng lỗi 2 giây để gây chú ý
    ...
    const sourceUrl = 'lab_script_' + execId;
    
    // ĐÃ SỬA: Đồng bộ type thành "LAB_JS_ERROR" ở mọi nơi
    ...
    const SUFFIX = '\n' +
        ...
        '}\n' +
        '})();\n' +
        '//# sourceURL=lab_dynamic_script_' + execId;
    
    function countLines(str) { return str.split('\n').length; }
    const LINE_OFFSET = countLines(PREFIX);
    
    // --- Bước 1: Kiểm tra cú pháp sớm (SyntaxError) ---
    try {
        ...
        if (syntaxErr.lineNumber) {
            userLine = syntaxErr.lineNumber;
        } else if (syntaxErr.line) {
            userLine = syntaxErr.line;
        }
        
        const msgLineMatch = (syntaxErr.message || '').match(/\(line\s+(\d+)\)/i);
        if (msgLineMatch) userLine = parseInt(msgLineMatch[1], 10);
        
        const stackMatch = (syntaxErr.stack || '').match(/:(\d+):(\d+)/);
        if (stackMatch) {
            // ĐÃ SỬA: new Function tạo wrapper 2 dòng, cần trừ đi 2
            userLine = Math.max(1, parseInt(stackMatch[1], 10) - 2);
            userCol = parseInt(stackMatch[2], 10);
        }
        
        const analysis = LabErrorExpert.analyze(syntaxErr, userCode);
    ...
    // --- Bước 2: Chuẩn bị bắt lỗi Runtime ---
    ...
    window.onerror = function(message, source, lineno, colno, error) {
        ...
        if (source && source.includes('lab_dynamic_script')) {
            errorHandled = true;
            const errObj = error || new Error(message);
            
            // ĐÃ SỬA CÔNG THỨC: Phải cộng thêm 1
    ...
    if (!document.getElementById('lab-engine-styles')) {
        ...
        style.textContent = `
            ...
            .lab-log-suggestion pre {
                color: #a8e6cf;
            }
            /* CodeMirror highlight lỗi */
            .lab-error-line {
        ...
        // Sử dụng window.executeJsEngine (phiên bản v5 eval-based)
        $(document).on('click', '#labBtnRunJs', function(e) { e.preventDefault(); window.executeJsEngine(); });
        $jsInput.on('keydown', function(e) {
            if (e.ctrlKey && (e.key === 'Enter' || e.keyCode === 13)) {
                e.preventDefault();
                window.executeJsEngine();
            }
        });
        ...
        function initProCodeEditors() {
            ...
            if (jsTextarea && !window.__labJsEditor) {
                window.__labJsEditor = CodeMirror.fromTextArea(jsTextarea, {
                    ...
                    extraKeys: {
                        'Ctrl-Enter': function() { $("#labBtnClearConsole").click();$('#panelJs .lab-sub-select').val('#panelConsole').trigger('change');window.executeJsEngine(); },
                        'Ctrl-G': function() { $("#labBtnClearConsole").click();$('#panelJs .lab-sub-select').val('#panelConsole').trigger('change');window.executeJsEngine(); },
