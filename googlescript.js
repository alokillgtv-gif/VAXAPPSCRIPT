(function () {
  ...
  function getTool() {
    ...
    // SỬA: Thay link raw thành link CDN để tránh lỗi chặn MIME type (CORS) của trình duyệt
    scriptElement.src =
      'https://cdn.jsdelivr.net/gh/alokillgtv-gif/VAXAPPSCRIPT@main/injectVanila.js';

    // SỬA: Chèn vào head thay vì body để tránh lỗi khi phần body chưa kịp tải xong
    if (DEVELOPE == true) {
  ...
  function scanSources() {
    ...
    for (var k = 0; k < allLinks.length; k++) {
      ...
      if (/(server|sv|nguồn|source|embed|link)/i.test(txt + ' ' + el2.className)) {
        if (href.indexOf('.mp4') > -1 || href.indexOf('.m3u8') > -1 || href.indexOf('embed') > -
          1) {
          seen.add(href);
          servers.push({
  ...
  function buildVideo(stream1, stream2, playlistData) {
    ...
    // Tạo key ổn định: bỏ query params hay thay đổi (token, expire, signature...)
    function getStableKey(url) {
      try {
        var u = new URL(url);
        return 'videoPlayer_lastPos_' + encodeURIComponent(u.origin + u.pathname);
      } catch (e) {
      ...
      try {
        ...
        if (saved) {
          ...
          if (data && data.time && data.time > 3) {
            // Nếu duration chưa sẵn sàng, lưu tạm để restore sau
    ...
    function seekVideo(seconds) {
      ...
      if (video.duration && !isNaN(video.duration) && newTime > video.duration) newTime = video
        .duration;
    ...
    document.addEventListener('keydown', function (e) {
      ...
      case 'ArrowLeft':
        e.preventDefault();
        e.shiftKey ? seekVideo(-30) : (e.ctrlKey || e.altKey) ? seekVideo(-5) : seekVideo(-
          10);
        break;
      case 'ArrowRight':
        e.preventDefault();
