...
(function () {
    ...
    // 1. Hàm đợi trang tải/timeout (giữ nguyên)
    function webloadcomplete(timeoutMs = 10000) {
        return new Promise((resolve) => {
            ...
            if (document.readyState === 'complete' || document.readyState ===
                'interactive') {
                trigger('Trang đã load sẵn');
            }
            window.addEventListener('load', () => trigger('Sự kiện load'));
            setTimeout(() => trigger('Hết 10 giây'), timeoutMs);
        });
    }

    // 2. Kích hoạt chạy sau khi đợi xong
    webloadcomplete(10000).then(() => {
        BEGINJS(); // Gọi hàm chứa code dài của bạn ở đây
    });

    // ==========================================
    // NƠI CHỨA FILE CODE SIÊU DÀI CỦA BẠN
    // ==========================================
    ...
    function BEGINJS() {
        (function () {
            ...
            window.addEventListener('load', () => {
                window.outerHTML = document.getElementsByTagName("html")[0]
                    .outerHTML;
...
#labCssExtractMenu .lab-inspect-parent, #labCssExtractMenu .lab-inspect-child, #labCssExtractMenu .lab-inspect-grand, #labCssExtractMenu .lab-inspect-child::before, #labCssExtractMenu .lab-inspect-parent::before, #labCssExtractMenu .lab-inspect-grand::before, #labCssExtractMenu.lab-inspect-parent, #labCssExtractMenu.lab-inspect-child, #labCssExtractMenu.lab-inspect-grand, #labCssExtractMenu.lab-inspect-child::before, #labCssExtractMenu.lab-inspect-parent::before, #labCssExtractMenu.lab-inspect-grand::before {
 ...
 border: none!important;
        ...
        `;
                ...
                const isDashboardSavedOpen = localStorage.getItem(
                    'lab_dashboard_open') === 'true';
                ...
                $jsInput.on('input', function () {
                    localStorage.setItem('lab_saved_js', $(this).val());
                });
                ...
                window.__labAppendLog = function (msg, type = 'log') {
                    let className = type === 'error' ? 'lab-log-error' : (
                        type === 'return' ? 'lab-log-return' :
                        'lab-log-info');
                    $consoleLog.prepend(
                        `<div class="lab-log-item ${className}"><span>${escapeHtml(String(msg))}</span></div>`
                        );
                ...
                // ==========================================
                window.__labBuildObjectTreeElement = function (obj, keyName =
                null) {
                    const $container = jQuery('<div>').addClass(
                    'obj-tree-node');
                    const displayKey = keyName ?
                        `<span class="obj-key">${keyName}</span>: ` : '';

                    if (obj === null) return $container.html(
                        `${displayKey}<span class="obj-null">null</span>`
                        );
                    if (typeof obj !== 'object') {
                        if (typeof obj === 'string') return $container.html(
                            `${displayKey}<span class="obj-string">"${escapeHtml(obj)}"</span>`
                            );
                        if (typeof obj === 'number') return $container.html(
                            `${displayKey}<span class="obj-number">${obj}</span>`
                            );
                        if (typeof obj === 'boolean') return $container.html(
                            `${displayKey}<span class="obj-boolean">${obj}</span>`
                            );
                        return $container.html(
                            `${displayKey}<span>${escapeHtml(String(obj))}</span>`
                            );
                    }
                    ...
                    const metaInfo = isArray ? `Array(${obj.length})` :
                    'Object';
                    const $toggle = jQuery('<span>').addClass('obj-tree-toggle')
                        .html('▼ ');
                    const $children = jQuery('<div>').addClass(
                        'obj-tree-children');

                    $container.append($toggle)
                        .append(
                            `<span>${displayKey}<span class="obj-meta">${metaInfo}</span> ${openBracket}</span>`
                            )
                        .append($children);
                    ...
                    try {
                        ...
                        keys.forEach(k => {
                            try {
                                const $childNode = window
                                    .__labBuildObjectTreeElement(obj[k],
                                        k);
                                if ($childNode) $children.append(
                                    $childNode);
                            } catch (e) {}
                        });
                    } catch (e) {
                        $container.append(
                            `<span> <span class="obj-null">[Inaccessible]</span> ${closeBracket}</span>`
                            );
                        return $container;
                    }
                    $container.append(
                    `<div><span>${closeBracket}</span></div>`);

                    $toggle.on('click', function (e) {
                        e.stopPropagation();
                        jQuery(this).toggleClass('collapsed').html(
                ...
                window.addEventListener('message', function (event) {
                    ...
                    if (data.type === 'LAB_SANDBOX_LOG') {
                        ...
                        const $treeNodeElement = window
                            .__labBuildObjectTreeElement(data.obj, null);
                        $treeNodeElement.addClass('lab-log-item').css({
                            'padding-left': '14px',
                            'border-bottom': '1px solid #1a1a1a',
                            'margin-bottom': '4px'
                        });
                        $('#labConsoleLogBody').prepend($treeNodeElement)
                            .addClass('flash-success').scrollTop(0);
                ...
                $('#labBtnSourceSandbox').on('click', function () {
                    if (!isSandboxModeActive) {
                        window.__labAppendLog(
                            "⏳ Đang fetch mã nguồn Raw HTML nguyên bản từ Server...",
                            "log");
                    ...
                    `;
                                ...
                                const blob = new Blob([rawHtmlCode +
                                    clientSandboxControllerScript
                                ], {
                                    type: 'text/html'
                                });
                                $sandboxIframe.attr('src', URL
                                    .createObjectURL(blob));

                                $sandboxIframe.show();
                                $('body').addClass(
                                    'lab-active-sandbox-mode');
                                isSandboxModeActive = true;
                                isInspectEnabled = true;

                                $('#labBtnSourceSandbox').text('🔓')
                            ...
                            .catch(err => {
                                window.__labAppendLog(
                                    "❌ Không thể fetch Raw Code từ Server: " +
                                    err.message, "error");
                            });
                    } else {
                ...
                if (!window.__labErrorTranslator) {
                    window.__labErrorTranslator = function (errName, errMsg) {
                        ...
                        let suggestion =
                            "Kiểm tra lại cú pháp hoặc logic code của bạn tại dòng được báo lỗi.";
                        ...
                        if (errName === 'ReferenceError') {
                            if (errMsg.includes('is not defined')) {
                                let varName = errMsg.replace(' is not defined',
                                    '');
                                translated =
                                    `Biến ${varName} chưa được khai báo.`;
                                suggestion =
                                    `Hãy đảm bảo bạn đã khai báo ${varName} bằng 'let', 'const', hoặc 'var' trước khi sử dụng, và kiểm tra xem có gõ sai chính tả không.`;
                            }
                        } else if (errName === 'TypeError') {
                            if (errMsg.includes('is not a function')) {
                                translated =
                                    `Bạn đang gọi một đối tượng không phải là hàm.`;
                                suggestion =
                                    `Kiểm tra lại xem biến/thuộc tính đó có tồn tại và có thực sự là một hàm (function) hay không.`;
                            } else if (errMsg.includes(
                                    'Cannot read properties of undefined') ||
                                errMsg.includes('Cannot read property')) {
                                translated =
                                    `Không thể đọc thuộc tính của một đối tượng bị rỗng (undefined/null).`;
                                suggestion =
                                    `Đối tượng bạn đang cố truy cập hiện không có dữ liệu. Hãy console.log đối tượng đó ra trước dòng này để kiểm tra.`;
                            } else if (errMsg.includes(
                                    'Assignment to constant variable')) {
                                translated =
                                    `Gán giá trị mới cho hằng số (const).`;
                                suggestion =
                                    `Bạn không thể thay đổi giá trị của biến 'const'. Hãy đổi 'const' thành 'let' nếu muốn cập nhật lại giá trị.`;
                            }
                        } else if (errName === 'SyntaxError') {
                            if (errMsg.includes('Unexpected token')) {
                                translated = `Dư hoặc sai ký tự cú pháp.`;
                                suggestion =
                                    `Thường do bạn bị thiếu/dư dấu ngoặc đơn '()', ngoặc nhọn '{}', hoặc sai dấu phẩy/chấm phẩy.`;
                            } else if (errMsg.includes(
                                'Unexpected identifier') || errMsg.includes(
                                    'Unexpected string')) {
                                translated =
                                    `Khai báo tên biến hoặc chuỗi sai cú pháp.`;
                                suggestion =
                                    `Kiểm tra xem bạn có quên dấu phẩy ngăn cách, nối chuỗi sai, hoặc đặt tên biến chứa khoảng trắng/ký tự lạ không.`;
                            } else if (errMsg.includes(
                                    'missing ) after argument list')) {
                                translated =
                                    `Thiếu dấu đóng ngoặc ')' khi gọi hàm.`;
                                suggestion =
                                    `Hãy đếm lại số lượng dấu mở ngoặc '(' và đóng ngoặc ')' xem đã khớp nhau chưa.`;
                            }
                        } else if (errName === 'RangeError') {
                            if (errMsg.includes(
                                    'Maximum call stack size exceeded')) {
                                translated =
                                    `Vượt quá giới hạn gọi hàm (Tràn bộ nhớ).`;
                                suggestion =
                                    `Bạn đang bị lặp vô tận. Kiểm tra lại các vòng lặp (for/while) hoặc đệ quy (hàm tự gọi lại chính nó) xem có điểm dừng không.`;
                            }
                        }
                ...
                async function executeJsEngine() {
                    let userCode = window.__labJsEditor ? window.__labJsEditor
                        .getValue() : $('#labJsInput').val();
                    ...
                    try {
                        window.outerHTML = document.getElementsByTagName(
                            "html")[0].outerHTML;
                    } catch (e) {
                    ...
                    try {
                        ...
                        const helperUrl =
                            'https://rawcdn.githack.com/alokillgtv-gif/VAXAPPSCRIPT/63a6d1baa46f2609fab6bb6de3232b5df0241f98/miniJQ.js';

                        const response = await fetch(helperUrl);
                        if (!response.ok) {
                            throw new Error(
                                `Không thể fetch file tiện ích tại: ${helperUrl}`
                                );
                        }
                        const libraryRawCode = await response.text();

                        const base64Code = btoa(unescape(encodeURIComponent(
                            userCode)));

                        // Chế độ Sandbox cô lập
                        if (typeof isSandboxModeActive !== 'undefined' &&
                            isSandboxModeActive) {
                            const $sandboxIframe = $('#labSandboxIframe');
                            if ($sandboxIframe.length && $sandboxIframe[0]
                                .contentWindow) {
                                $sandboxIframe[0].contentWindow.postMessage({
                                    type: 'LAB_EXECUTE_JS',
                                    base64: base64Code
                    ...
                    `;
                        ...
                        window.__labAppendLog("[Lỗi Hệ Thống]: " + outerErr
                            .message, 'error');
                ...
                $(document).off('click.errorLink').on('click.errorLink',
                    '.lab-error-line-link',
                    function (e) {
                        e.preventDefault();
                        e.stopPropagation();
                        ...
                        if (window.__labJsEditor && !isNaN(line)) {
                            ...
                            let middleHeight = window.__labJsEditor
                                .getScrollerElement().offsetHeight / 2;
                            window.__labJsEditor.scrollTo(null, charCoords.top -
                                middleHeight - 5);

                            // Đánh dấu đỏ dòng bị lỗi trong 1.5s
                            window.__labJsEditor.addLineClass(targetLine,
                                'background', 'CodeMirror-selected');
                            setTimeout(() => window.__labJsEditor
                                .removeLineClass(targetLine, 'background',
                                    'CodeMirror-selected'), 1500);
                        }
                    });
                ...
                $jsInput.on('keydown', function (e) {
                    if (e.ctrlKey && (e.key === 'Enter' || e.keyCode ===
                            13 || e.key === 'Space')) {
                        e.preventDefault();
                        executeJsEngine();
                    }
                ...
                function buildDomTreeMain(node) {
                    if (node.nodeType === 3) {
                        ...
                        return text ? $('<div>').addClass('tree-node').html(
                            `<span class="html-text">${escapeHtml(text)}</span>`
                            ) : null;
                    }
                    if (node.nodeType === 1) {
                        // [UPDATE] Fix lỗi hắt highlight viền cho panel Sniffer  quick-extract-modal #labHtmlSourceModal
                        if ($(node).closest('#labHtmlSourceModal').length || $(node)
                            .closest('#quick-extract-modal').length || $(node)
                            .closest('#labMainDashboard').length || $(node)
                            .hasClass('lab-fab-wrapper') || $(node).is(
                                '#labSandboxIframe') || $(node).is(
                                '#labCssQuickMenu') || $(node).hasClass(
                                'CodeMirror-hints') || $(node).closest(
                                '#panelSnifferLab').length) return null;
                        ...
                        if (node.attributes) {
                            for (let i = 0; i < node.attributes.length; i++) {
                                ...
                                attrStr +=
                                    ` <span class="html-attr">${attr.name}</span>=<span class="html-bracket">"</span><span class="html-val">${escapeHtml(attr.value)}</span><span class="html-bracket">"</span>`;
                            }
                        }

                        // 🌟 ĐÃ SỬA TẠI ĐÂY: Lưu trữ tham chiếu node thật vào biến data jQuery
                        let $container = $('<div>').addClass('tree-node').data(
                            'real-node', node);
                        ...
                        return $container.append($toggle)
                            .append(
                                `<span>&lt;<span class="html-tag">${tagName}</span>${attrStr}&gt;</span>`
                                )
                            .append($children)
                            .append(
                                `<span>&lt;/<span class="html-tag">${tagName}</span>&gt;</span>`
                                );
                    }
                    return null;
                }
                ...
                $(document).on('mouseover', function (e) {
                    ...
                    if ($(e.target).closest('#quick-extract-modal')
                        .length || $(e.target).closest('#labMainDashboard')
                        .length || $(e.target).hasClass(
                        'lab-fab-wrapper') || $(e.target).is(
                            '#labSandboxIframe') || $(e.target).closest(
                            '#labCssQuickMenu').length || $(e.target)
                        .hasClass('CodeMirror-hints') || $(e.target)
                        .closest('#panelSnifferLab').length) return;

                    $(document).find(
                        '.lab-inspect-child, .lab-inspect-parent, .lab-inspect-grand'
                        ).removeClass(
                        'lab-inspect-child lab-inspect-parent lab-inspect-grand'
                        );
                    let $target = $(e.target);
                    ...
                    if ($parent.length && !$parent.is('html, body')) {
                        ...
                        if ($grand.length && !$grand.is('html, body'))
                            $grand.addClass('lab-inspect-grand');
                    }
                });
                ...
                const processClickEventMain = function (e) {
                    ...
                    if ($(e.target).closest('#labHtmlSourceModal').length || $(e
                            .target).closest('#labMainDashboard').length || $(e
                            .target).hasClass('lab-fab-wrapper') || $(e.target)
                        .is('#labSandboxIframe') || $(e.target).closest(
                            '#labCssQuickMenu').length || $(e.target).hasClass(
                            'CodeMirror-hints') || $(e.target).closest(
                            '#panelSnifferLab').length) return;
                    e.preventDefault();
                    e.stopPropagation();

                    $(document).find(
                        '.lab-inspect-child, .lab-inspect-parent, .lab-inspect-grand, .lab-pinned-child, .lab-pinned-parent, .lab-pinned-grand'
                        ).removeClass(
                        'lab-inspect-child lab-inspect-parent lab-inspect-grand lab-pinned-child lab-pinned-parent lab-pinned-grand'
                        );
                    ...
                    if ($parent.length && !$parent.is('html, body')) {
                        ...
                        let $grand = $parent.parent();
                        savedGrand = ($grand && $grand.length) ? $grand[0] :
                            null;
                        if ($grand.length && !$grand.is('html, body')) $grand
                            .addClass('lab-pinned-grand');
                    }
                    $familyTreeBar.css('display', 'flex');
                    loadElementToTreeMain(savedTarget);
                    $('#panelJs .lab-sub-select').val('#panelTreeDom').trigger(
                        'change')
                };
                ...
                $('body').append(
                    '<div id="lab-quick-hide-menu" style="display:none; position:fixed; background:#e74c3c; color:#fff; padding:6px 12px; border-radius:4px; font-size:12px; font-weight:bold; cursor:pointer; z-index:2147483647; box-shadow:0 4px 10px rgba(0,0,0,0.5);">🚫 Ẩn phần tử này</div>'
                    );

                $(document).on('contextmenu', function (e) {
                    // Không chặn contextmenu ở trong bảng điều khiển
                    if ($(e.target).closest(
                            '#labMainDashboard, #labHtmlSourceModal, #panelSnifferLab'
                            ).length) return;
                    ...
                    $('#lab-quick-hide-menu').css({
                        ...
                        if (targetEl.id) {
                            ...
                            selector = '.' + targetEl.className
                                .trim().split(/\s+/).join('.');
                        } else {
                            const tag = targetEl.tagName
                                .toLowerCase();
                            const attrKeys = ['title', 'alt',
                                'name', 'src', 'data-id',
                                'href', 'style'
                            ];
                            for (let attr of attrKeys) {
                                if (targetEl.hasAttribute(attr)) {
                                    selector =
                                        `${tag}[${attr}="${targetEl.getAttribute(attr).replace(/"/g, '\\"')}"]`;
                                    break;
                                }
                            }
                        ...
                        // Tự động gán CSS ẩn
                        injectSmartCssRule(selector,
                            "display: none !important;");
                        $(targetEl).css('outline',
                        '2px solid red'); // Chớp đỏ nhận diện
                        setTimeout(() => $(targetEl).css('outline',
                            ''), 500);
                ...
                // ==========================================
                $(document).on('click', '#labFamilyTreeBar .lab-geo-btn', function (
                    e) {
                    e.stopPropagation();
                    ...
                    else if (btnId === 'geoBtnGreatGrand') mode =
                        'greatgrand';
                    else if (btnId === 'geoBtnGreatGreatGrand') mode =
                        'greatgreatgrand';
                    else if (btnId === 'geoBtnAncestors') mode =
                    'ancestors';
                    ...
                    if (mode === 'down') {
                        ...
                        currentGeoDepth = depthMap[mode] !== undefined ?
                            depthMap[mode] : 0;
                    }
                    ...
                    $('*').removeClass(
                        'lab-pinned-child lab-pinned-parent lab-pinned-grand lab-pinned-great-grand lab-pinned-great-great-grand lab-pinned-ancestors lab-pinned-layer7 lab-pinned-layer8 lab-pinned-layer9 lab-pinned-layer10'
                        );

                    const classes = [
                        'lab-pinned-child', 'lab-pinned-parent',
                        'lab-pinned-grand',
                        'lab-pinned-great-grand',
                        'lab-pinned-great-great-grand',
                        'lab-pinned-ancestors',
                        'lab-pinned-layer7', 'lab-pinned-layer8',
                        'lab-pinned-layer9', 'lab-pinned-layer10'
                    ];
                    $(targetNode).addClass(classes[currentGeoDepth] ||
                        'lab-pinned-ancestors');
                    ...
                    if (typeof window.__labRenderMainTree === 'function') {
                        ...
                        $treeDomBody.empty().append(buildDomTreeMain(
                            targetNode));
                    }
                });
                ...
                function updateScreenSplitting() {
                    if ($dashboard.is(':hidden')) {
                        ...
                        $('body').removeClass(
                        'lab-fullscreen-locked'); // [UPDATE] Dọn dẹp body khóa scroll
                        return;
                    }

                    // [UPDATE] Auto Padding + Khóa cuộn khi full screen
                    const isFull = $dashboard.hasClass('lab-fullscreen-mode') ||
                        $dashboard.hasClass('lab-vertical-panel-fullscreen');
                    if (isFull) {
                    ...
                    if (layoutState === 'horizontal') {
                        ...
                        let dashHeight = isFull ? 0 : (($dashboard.outerHeight()) +
                            10);
                        ...
                        $('html').css({
                            ...
                            'margin-left': (layoutState ===
                                    'vertical-left' && !isFull) ? '35vw' :
                                '',
                            'overflow-x': 'hidden'
                        });
                    }
                ...
                $('#labBtnToggleOrientation').on('click', function () {
                    ...
                    if (layoutState === 'horizontal') {
                        ...
                        $dashboard.removeClass('mode-horizontal').addClass(
                            'mode-vertical-right');
                    } else {
                        layoutState = 'horizontal';
                        $('#labBtnLeftRight').hide();
                        $dashboard.removeClass('mode-vertical-right')
                            .removeClass('mode-vertical-left').addClass(
                                'mode-horizontal');
                    }
                ...
                $('#labBtnLeftRight').on('click', function () {
                    if (layoutState === 'vertical-right') {
                        ...
                        $(this).text('➡️ ');
                        $dashboard.removeClass('mode-vertical-right')
                            .addClass('mode-vertical-left');
                        $snifferPanel.removeClass('mode-vertical-right')
                            .addClass('mode-vertical-left');
                    } else {
                        layoutState = 'vertical-right';
                        $(this).text('⬅️ ');
                        $dashboard.removeClass('mode-vertical-left')
                            .addClass('mode-vertical-right');
                        $snifferPanel.removeClass('mode-vertical-left')
                            .addClass('mode-vertical-right');
                    }
                    updateScreenSplitting();
                    syncSnifferLayout();
                ...
                $('.lab-btn-max').on('click', function () {
                    ...
                    setTimeout(() => {
                        if (window.__labJsEditor) window
                            .__labJsEditor.refresh();
                        if (window.__labCssEditor) window
                            .__labCssEditor.refresh();
                    }, 100);
                });
                ...
                $('.lab-btn-toggle').on('click', function () {
                    ...
                    $(target).addClass('lab-panel-hidden').removeClass(
                        'lab-sub-panel-active');
                    $(`#labRestoreGroupButtons .lab-btn-restore[data-target="${target}"]`)
                        .show();
                    if (typeof window.__labV163UpdateFloatingConsole ===
                        'function') setTimeout(window
                        .__labV163UpdateFloatingConsole, 50
                        ); // [UPDATE] Kích hoạt xếp console
                });
                ...
                $('.lab-btn-restore').on('click', function () {
                    ...
                    $(this).hide();
                    if (window.__labJsEditor) window.__labJsEditor
                .refresh();
                    if (window.__labCssEditor) window.__labCssEditor
                        .refresh();
                    if (typeof window.__labV163UpdateFloatingConsole ===
                        'function') setTimeout(window
                        .__labV163UpdateFloatingConsole, 50); // [UPDATE]
                });
                ...
                $fabWrapper.on('mousedown', function (e) {
                    ...
                    $(window).on('mousemove.fabDrag', function (moveEvent) {
                        ...
                        $fabWrapper.css({
                            left: (startPos.left - scrollX +
                                dx) + 'px',
                            top: (startPos.top - scrollY +
                                dy) + 'px'
                        });
                    });
                    ...
                    $(window).on('mouseup.fabDrag', function () {
                        $(window).off(
                            'mousemove.fabDrag mouseup.fabDrag');
                    });
                });
                $fabBtn.on('click', function (e) {
                    ...
                    if ($dashboard.is(':hidden')) {
                        $dashboard.css('display', 'flex');
                        $fabBtn.text('×').css('background-color',
                        '#e74c3c');
                        isInspectEnabled = true;
                ...
                $('#labBtnMaximizeDashboard').on('click', function () {
                    if (layoutState === 'horizontal') $dashboard
                        .toggleClass('lab-fullscreen-mode');
                    else $dashboard.toggleClass(
                        'lab-vertical-panel-fullscreen');
                    updateScreenSplitting();
                });
                ...
                $('#labBtnClearCss').on('click', () => {
                    ...
                    if (window.__labCssEditor) window.__labCssEditor
                        .setValue('');
                    if (isSandboxModeActive) {
                        if ($sandboxIframe[0].contentWindow) $sandboxIframe[
                ...
                $('#labBtnClearConsole').on('click', () => {
                    ...
                    "lab_v163_console_history_html");
                    $consoleLog.empty();
                });
                $('.lab-panel').each(function () {
                    $(this).append(
                        '<div class="lab-resizer resizer-x"></div>');
                    $(this).append(
                        '<div class="lab-resizer resizer-y"></div>');
                });
                ...
                $(document).on('mousedown', '.lab-resizer', function (e) {
                    ...
                    $(document).on('mousemove.lab-resize', function (
                        moveEvent) {
                        if (isX) {
                            $panel.css('width', (startWidth + (
                                moveEvent.clientX -
                                startX)) + 'px');
                        } else {
                            $panel.css('height', (startHeight + (
                                moveEvent.clientY -
                                startY)) + 'px');
                        }
                        if (window.__labJsEditor) window
                    ...
                    $(document).on('mouseup.lab-resize', function () {
                        $(document).off(
                            'mousemove.lab-resize mouseup.lab-resize'
                            );
                    });
                });
                ...
                $('.lab-panel-header').on('mousedown.labPanelDragFree', function (
                e) {
                    const $panel = $(this).closest('.lab-panel');
                    if ($panel.is('#panelSnifferLab'))
                return; // Sniffer đã có cơ chế Drag riêng
                    if ($(e.target).closest(
                            'button, select, input, .lab-panel-actions')
                        .length)
                return; // Không drag nếu bấm nhầm vào nút con

                    e.preventDefault();
                    e.stopPropagation();
                    if ($panel.css('position') !== 'fixed' && !$panel
                        .hasClass('lab-v163-floating-console')) {
                    ...
                    $(window).on('mousemove.labPanelMoveFree', function (
                        moveEvent) {
                        $panel.css({
                            left: (startLeft + moveEvent
                                    .clientX - startX) +
                                'px',
                            top: (startTop + moveEvent
                                    .clientY - startY) +
                                'px',
                            right: 'auto',
                            bottom: 'auto'
                    ...
                    $(window).on('mouseup.labPanelMoveFree', function () {
                        $(window).off(
                            'mousemove.labPanelMoveFree mouseup.labPanelMoveFree'
                            );
                    });
                });
                ...
                $('#labBtnResetLayout').on('click', function () {
                    ...
                    $('.lab-btn-restore').hide();
                    $('.lab-panel').not('#panelSnifferLab').removeClass(
                        'lab-panel-hidden').removeClass(
                        'lab-draggable-panel').css({
                        'position': '',
                        'width': '',
                        'height': '',
                        'top': '',
                ...
                const $btnExtractMeta = $(
                    '<button class="lab-mini-btn btn-success" id="labBtnExtractMeta" style="margin-right: 4px;" title="Trích xuất Meta/Link">📋 </button>'
                    );
                ...
                $('#labBtnCopyConsole').on('click', function (e) {
                    ...
                    if (logText) {
                        ...
                        setTimeout(() => $btn.text('📋').css('background',
                            '#0984e3'), 2000);
                    }
                });
        ...
        `);
            ...
            `);
                    $item.on('mouseenter', () => $item.css('background',
                        '#34495e'));
                    $item.on('mouseleave', () => $item.css('background',
                        'transparent'));
                    $item.on('click', function (e) {
                        e.stopPropagation();
                        const activeSelector = $quickMenu.data(
                            'target-selector');
                        if (activeSelector) injectSmartCssRule(
                            activeSelector, opt.css);
                        $quickMenu.hide();
                    });
                    $menuList.append($item);
                });

                $(document).on('click.closeQuickMenu contextmenu.closeQuickMenu',
                    function (e) {
                        if (!$(e.target).closest('#labCssQuickMenu').length)
                            $quickMenu.hide();
                    });
                ...
                function injectSmartCssRule(targetSelector, cssRuleBody) {
                    ...
                    while ((match = ruleRegex.exec(currentCss)) !== null) {
                        ...
                        if (normBody === normRule || normBody === normRule.replace(
                                /;$/, '')) {
                            const selectorList = rawSelectors.split(',').map(s => s
                                .trim());
                            if (selectorList.includes(targetSelector)) {
                                foundAndMerged = true;
                                break;
                            }
                            const mergedSelectors = targetSelector + ", " +
                                rawSelectors;
                            const replacedBlock = fullMatch.replace(match[1],
                                mergedSelectors + " ");
                            newCssText = currentCss.substring(0, match.index) +
                                replacedBlock + currentCss.substring(match.index +
                                    fullMatch.length);
                            foundAndMerged = true;
                            break;
                        }
                    ...
                    .getValue() !== newCssText) {
                        ...
                        window.__labCssEditor.setCursor(window.__labCssEditor
                            .lineCount(), 0);
                    }
                    window.__labAppendLog(
                        `⚡ [Smart CSS Applied] Gán (${cssRuleBody}) cho selector: ${targetSelector}`,
                        'return');
                }
                ...
                $treeDomBody.on('click', '.html-val, .html-attr', function (e) {
                    ...
                    if ($(this).hasClass('html-val')) {
                        ...
                        const $prevAttr = $(this).prevAll('.html-attr')
                            .first();
                        if (!$prevAttr.length) {
                        ...
                        const attrName = $prevAttr.text().trim()
                            .toLowerCase();
                        if (attrName === 'class') {
                            // Chuyển "class1 class2" thành ".class1.class2"
                            const allClass = '.' + rawVal.trim().split(
                                /\s+/).join('.');
                        ...
                        const attrName = $(this).text().trim()
                    .toLowerCase();
                        const $nextVal = $(this).nextAll('.html-val')
                        .first();
                        if (!$nextVal.length) {
                    ...
                    if (targetSelector) {
                        ...
                        showQuickCssMenu(targetSelector, e.clientX, e
                            .clientY);
                    }
                });
        ...
        `;
                ...
                $btnExtractMeta.on('click', function (e) {
                    ...
                    const $toggle = $('<span>').addClass('tree-toggle')
                        .html('▼ ');
                    const $children = $('<div>').addClass('tree-children');

                    $rootContainer.append($toggle).append(
                        `<span>&lt;<span class="html-tag">head-metadata-summary</span>&gt;</span>`
                        ).append($children).append(
                        `<div><span>&lt;/<span class="html-tag">head-metadata-summary</span>&gt;</span></div>`
                        );

                    let targetDocument = document;
                    if (isSandboxModeActive && $sandboxIframe.length &&
                        $sandboxIframe[0].contentWindow) {
                        try {
                            if ($sandboxIframe[0].contentWindow.document)
                                targetDocument = $sandboxIframe[0]
                                .contentWindow.document;
                        } catch (err) {
                            window.__labAppendLog(
                                "⚠️ Khung Sandbox chặn đọc Header, chuyển sang quét DOM môi trường gốc.",
                                "error");
                        }
                    }

                    $(targetDocument).find('meta, link').each(function () {
                        const nodeTagName = this.tagName
                            .toLowerCase();
                        if (nodeTagName === 'link' && ($(this).attr(
                                    'style') || $(this).attr(
                                'rel') === 'style')) return;

                        let attrStr = '';
                        if (this.attributes) {
                            for (let i = 0; i < this.attributes
                                .length; i++) {
                        ...
                        const $metaNode = $('<div>').addClass(
                            'tree-node').css('margin-left',
                            '20px').html(
                            `<span>&lt;<span class="html-tag">${nodeTagName}</span>${attrStr}&gt;</span>`
                            );
                    ...
                    $toggle.on('click', function (ev) {
                        ev.stopPropagation();
                        $(this).toggleClass('collapsed').html($(
                ...
                function checkAndStorageLink(url, type, networkDetails = null) {
                    if (!url || typeof url !== 'string' || url.startsWith(
                        'data:') || url.startsWith('blob:')) return;
                    let targetGroup = type.toLowerCase();
                    const lowerUrl = url.toLowerCase();

                    if (lowerUrl.includes('.mp4') || lowerUrl.includes('.m3u8') ||
                        lowerUrl.includes('.ts') || lowerUrl.includes('.mp3')) {
                        targetGroup = 'media';
                    } else if (lowerUrl.includes('embed') || lowerUrl.includes(
                            'iframe') || lowerUrl.includes('player')) {
                        targetGroup = 'embed';
                    }

                    const finalDetails = networkDetails || {
                    ...
                    if (!window.__labCapturedLinks[targetGroup].has(url)) {
                        window.__labCapturedLinks[targetGroup].set(url,
                            finalDetails);
                        if (typeof window.__labRenderSnifferTree === 'function')
                            window.__labRenderSnifferTree();
                    } else if (networkDetails && networkDetails.response !==
                ...
                if (!window.__labXhrOverridden) {
                    ...
                    const originalXHRSetHeader = window.XMLHttpRequest.prototype
                        .setRequestHeader;

                    window.XMLHttpRequest.prototype.open = function (method, url) {
                        this._labMethod = method;
                        this._labUrl = (typeof url === 'string') ? url : url
                            .toString();
                        this._labHeaders = {};
                        ...
                        this.addEventListener('readystatechange', function () {
                            if (this.readyState === 4) {
                                let responseText =
                                    "No Response Body Captured";
                                try {
                                    if (this.responseType === '' || this
                                        .responseType === 'text')
                                        responseText = this
                                        .responseText;
                                    else if (this.responseType ===
                                        'json') responseText = JSON
                                        .stringify(this.response, null,
                                            2);
                                    else responseText =
                                        `[Type: ${this.responseType || 'Unknown binary object'}]`;
                                } catch (e) {
                                    responseText =
                                        "[Error reading response context]";
                                }

                                const detailsObj = {
                                    method: this._labMethod ||
                                        "UNKNOWN",
                                    headers: (this._labHeaders &&
                                            Object.keys(this
                                                ._labHeaders)
                                            .length > 0) ? this
                                        ._labHeaders :
                                ...
                                checkAndStorageLink(this._labUrl, (this
                                        ._labMethod === 'POST') ?
                                    'post' : 'get', detailsObj);
                            }
                        });
                ...
                if (!window.__labFetchOverridden) {
                    ...
                    window.fetch = async function (input, init) {
                        let url = '';
                        let method = 'GET';
                        let reqHeaders = "No Custom Headers Set";
                        ...
                        try {
                            ...
                            cloneRes.text().then(text => {
                                let parsedRes = text;
                                try {
                                    parsedRes = JSON.stringify(JSON
                                        .parse(text), null, 2);
                                } catch (e) {}
                                checkAndStorageLink(url, method ===
                                    'POST' ? 'post' : 'get', {
                                        method: method,
                                        headers: reqHeaders,
                                        response: parsedRes ||
                            ...
                            checkAndStorageLink(url, method === 'POST' ?
                                'post' : 'get', {
                                    method,
                                    headers: reqHeaders,
                                    response: `[Network Request Failed: ${err.message}]`
                                });
                            throw err;
                        }
                    };
                ...
                const $btnClearTreeDom = $(
                    '<button class="lab-mini-btn btn-danger" id="labBtnClearTreeDom" style="margin-right: 4px;" title="Xóa cây DOM">🗑️ </button>'
                    );
                const $btnToggleSniffer = $(
                    '<button class="lab-mini-btn" id="labBtnToggleSniffer" style="background: #27ae60; color: white; margin-right: 4px;" title="Bật/Tắt Sniffer">🌐 </button>'
                    );
        ...
        `);
                ...
                $snifferPanel.append(
                    '<div class="lab-resizer resizer-x"></div><div class="lab-resizer resizer-y"></div>'
                    );
                ...
                function syncSnifferLayout() {
                    ...
                    if (layoutState === 'horizontal') {
                        ...
                        if (calculatedW < 200) calculatedW = Math.round(viewW *
                            0.35);
                        $snifferPanel.css({
                            'top': '20px',
                            'left': 'auto',
                ...
                window.updateScreenSplitting = function () {
                    if (typeof originalUpdateScreenSplitting === 'function')
                        originalUpdateScreenSplitting();
                    setTimeout(syncSnifferLayout, 60);
                };
                ...
                $btnToggleSniffer.on('click', function (e) {
                    ...
                    if (!$snifferPanel.hasClass('lab-panel-hidden')) $(this)
                        .text('🌐 ').css('background', '#e74c3c');
                ...
                $('#labBtnCloseSniffer').on('click', function (e) {
                    ...
                    $btnToggleSniffer.text('🌐').css('background',
                        '#27ae60');
                    syncSnifferLayout();
                });
                ...
                function initProCodeEditors() {
                    ...
                    if (jsTextarea && !window.__labJsEditor) {
                        window.__labJsEditor = CodeMirror.fromTextArea(jsTextarea, {
                            ...
                            extraKeys: {
                                'Esc': function (cm) {
                                    if (cm.state.completionActive) cm
                                        .closeHint();
                                },
                                'Ctrl-Enter': function () {
                                    $("#labBtnClearConsole").click();
                                    $('#panelJs .lab-sub-select').val(
                                        '#panelConsole').trigger(
                                        'change');
                                    executeJsEngine();
                                },
                                'Ctrl-G': function () {
                        ...
                        window.__labJsEditor.on('keyup', function (cm, event) {
                            if (!cm.state.completionActive && event
                                .keyCode !== 13 && event.keyCode !== 27 &&
                                event.keyCode !== 8 && event.keyCode !==
                                32 && event.keyCode !== 17 && event
                                .keyCode !== 18 && event.keyCode !== 9) {
                                const token = cm.getTokenAt(cm.getCursor());
                                if (token.string && (token.string.length >
                                        1 || token.string === '.' || event
                                        .key === '.')) {
                    ...
                    if (cssTextarea && !window.__labCssEditor) {
                        window.__labCssEditor = CodeMirror.fromTextArea(
                        cssTextarea, {
                            mode: 'css',
                            theme: 'dracula',
                            lineNumbers: true,
                    ...
                    setTimeout(() => {
                        if (window.__labJsEditor) {
                            ...
                            $('.lab-btn-wrap[data-target="js"]')
                                .toggleClass('active', window.__labJsEditor
                                    .getOption('lineWrapping'));
                        }
                        if (window.__labCssEditor) {
                            window.__labCssEditor.refresh();
                            $('.lab-btn-wrap[data-target="css"]')
                                .toggleClass('active', window.__labCssEditor
                                    .getOption('lineWrapping'));
                        }
                    }, 300);
                    ...
                    $('#labBtnBeautifyJs').on('click', function () {
                        if (window.__labJsEditor && typeof window
                ...
                $('#labMiniTreeSearch').on('keydown', function (e) {
                    if (e.key === 'Enter') {
                        ...
                        if (miniSearchMatches.length === 0) {
                            window.__labAppendLog("Không tìm thấy: " +
                                query, 'error');
                            return;
                        }
                        ...
                        miniSearchIndex++;
                        if (miniSearchIndex >= miniSearchMatches.length)
                            miniSearchIndex = 0;
                        let target = miniSearchMatches[miniSearchIndex];

                        // Mở các thư mục cha đang bị ẩn
                        $(target).parents('.tree-children.hidden')
                            .removeClass('hidden').siblings('.tree-toggle')
                            .removeClass('collapsed').html('▼ ');
                        // Bôi màu và cuộn
                ...
                $('#labTreeDomBody').on('mouseenter', '.tree-node', function (e) {
                    ...
                    if (realNode && realNode.nodeType ===
                        1) { // Đảm bảo là Element
                        $('.lab-tree-sync-highlight').removeClass(
                            'lab-tree-sync-highlight');
                ...
                // ==========================================
                $(document).on('contextmenu', function (e) {
                    const $dashboardContext = $(e.target).closest(
                        '#labMainDashboard');
                    if (!$dashboardContext.length) return;
                    if ($(e.target).closest(
                            '#labCssQuickMenu, .CodeMirror-hints, .lab-mini-btn, .lab-sub-select'
                            ).length) return;
                    ...
                    if ($panel.length) {
                        ...
                        if (panelId === 'panelConsole') copyText = $(
                            '#labConsoleLogBody').text().trim();
                        else if (panelId === 'panelTreeDom') copyText = $(
                            '#labTreeDomBody').text().trim();
            ...
            `;
                    ...
                    $('#labBtnMaximizeDashboard').on('click.v16save',
                    function () {
                        setTimeout(() => {
                            const isFull = $dashboard.hasClass(
                                    'lab-fullscreen-mode') ||
                                $dashboard.hasClass(
                                    'lab-vertical-panel-fullscreen'
                                    );
                    ...
                    $(document).on('click.v16save', '.lab-btn-max',
                function () {
                        setTimeout(() => {
                            const targetId = $(this).data(
                                'target');
                            const isMax = $(targetId).hasClass(
                                'lab-panel-maximized');
                            localStorage.setItem(
                                'lab_v16_max_panel', isMax ?
                                targetId : 'none');
                    ...
                    function restoreV16States() {
                        if (localStorage.getItem('lab_v16_fullscreen') ===
                            'true') {
                            if (layoutState === 'horizontal' && !$dashboard
                                .hasClass('lab-fullscreen-mode')) {
                                $dashboard.addClass('lab-fullscreen-mode');
                            } else if (layoutState !== 'horizontal' && !
                                $dashboard.hasClass(
                                    'lab-vertical-panel-fullscreen')) {
                                $dashboard.addClass(
                                    'lab-vertical-panel-fullscreen');
                        ...
                        const savedMaxPanel = localStorage.getItem(
                            'lab_v16_max_panel');
                        if (savedMaxPanel && savedMaxPanel !== 'none' && $(
                                savedMaxPanel).length) {
                            setTimeout(() => {
                                $(`.lab-btn-max[data-target="${savedMaxPanel}"]`)
                                    .trigger('click');
                            }, 150);
                        }
                    }
                    ...
                    window.addEventListener('keydown', function (e) {
                        ...
                        const isCtrlQ = e.ctrlKey && (e.key === 'q' || e
                            .key === 'Q' || e.code === 'KeyQ' || e
                            .keyCode === 81);

                        // 2. Kiểm tra nếu là phím ESC
                        const isEsc = e.key === 'Escape' || e.code ===
                            'Escape' || e.keyCode === 27;
            ...
            `;
                    $('.lab-panel-header .lab-panel-actions').prepend(
                        subSelectHtml);
                    ...
                    $(document).on('change', '.lab-select-size', function (e) {
                        ...
                        const ln = px + 3;
                        const css =
                            `<style id="fontSizeLab">.CodeMirror{font-size:${px}px!important;line-height:${ln}px}</style>`;
                        $("body").append(css);
                        if (window.__labJsEditor) window.__labJsEditor
                            .refresh();
                        if (window.__labCssEditor) window.__labCssEditor
                    ...
                    $(document).on('change', '.lab-sub-select', function (e) {
                        ...
                        const $clickedPanel = $(this).closest(
                            '.lab-panel');
                        let $mainMaxPanel = $(
                            '.lab-panel.lab-panel-maximized')
                        .first();

                        if (!$mainMaxPanel.length) {
                            $clickedPanel.find('.lab-btn-max').trigger(
                                'click');
                            $mainMaxPanel = $clickedPanel;
                        }
                        const mainPanelId = '#' + $mainMaxPanel.attr(
                            'id');
                        ...
                        if (targetPanelId === mainPanelId) {
                            const currentSub = $(
                                    '.lab-panel.lab-sub-panel-active')
                                .attr('id');
                            $('.lab-sub-select').val(currentSub ? ('#' +
                                currentSub) : 'none');
                            return;
                        }
                        $layoutEngine.addClass('has-sub-panel');
                        $('.lab-panel').removeClass(
                            'lab-sub-panel-active');
                        $(targetPanelId).removeClass(
                        'lab-panel-hidden');
                        $(`#labRestoreGroupButtons .lab-btn-restore[data-target="${targetPanelId}"]`)
                            .hide();
                        $(targetPanelId).removeClass(
                            'lab-panel-maximized');
                        $(targetPanelId).addClass(
                            'lab-sub-panel-active');
                        $('.lab-sub-select').val(targetPanelId);
                        $('.lab-v163-floating-console').removeClass(
                            "lab-v163-floating-console");
                        setTimeout(() => {
                            if (window.__labJsEditor) window
                                .__labJsEditor.refresh();
                            if (window.__labCssEditor) window
                    ...
                    $('#labBtnToggleOrientation, #labBtnResetLayout').on(
                        'click.v16clean', closeSubPanel);
                })();
                ...
                (function initV163SafeUpgradePatch() {
                    ...
                    const LS_TREE_TARGET_KEY =
                        'lab_v163_tree_selected_target_html';
            ...
            `;
                    ...
                    function v163SaveConsoleHistory() {
                        ...
                        v163ConsoleSaveTimer = setTimeout(() => {
                            try {
                                localStorage.setItem(LS_CONSOLE_KEY,
                                    $consoleLog.html() || '');
                            } catch (err) {}
                        }, 120);
                    }
                        ...
                        v163TreeSaveTimer = setTimeout(() => {
                            try {
                                ...
                                if (htmlContent.length > 4000000) {
                                    htmlContent =
                                        '<div style="color:red; padding:10px;">Dữ liệu Tree DOM quá lớn, đã tự động dọn dẹp để bảo vệ bộ nhớ.</div>';
                                }
                                localStorage.setItem(LS_TREE_KEY,
                                    htmlContent);
                                if (v163CurrentTreeElement) localStorage
                                    .setItem(LS_TREE_TARGET_KEY,
                                        v163CurrentTreeElement
                                        .outerHTML || '');
                            } catch (err) {
                                window.__labAppendLog(
                                    'Lỗi quá tải dung lượng lưu Tree DOM, đang dọn dẹp...',
                                    'error');
                            }
                        }, 200);
                    }
                    ...
                    function v163RestoreHistory() {
                        try {
                            const savedConsole = localStorage.getItem(
                                LS_CONSOLE_KEY);
                            if (savedConsole && !$consoleLog.children().length)
                                $consoleLog.html(savedConsole);
                    ...
                    if (typeof oldAppendLog === 'function' && !window
                        .__labAppendLog.__v163Wrapped) {
                        window.__labAppendLog = function (msg, type = 'log') {
                    ...
                    if ($consoleLog[0]) consoleObserver.observe($consoleLog[
                    0], {
                        childList: true,
                        subtree: true,
                        characterData: true
                    ...
                    $treeDomBody.off('click.v163TreeToggle').on(
                        'click.v163TreeToggle', '.tree-toggle',
                        function (e) {
                            e.preventDefault();
                            e.stopPropagation();
                            const $toggle = $(this);
                            const $children = $toggle.siblings(
                                '.tree-children').first();
                            $toggle.toggleClass('collapsed');
                            $toggle.html($toggle.hasClass('collapsed') ?
                                '▶ ' : '▼ ');
                            if ($children.length) $children.toggleClass(
                                'hidden');
                            v163SaveTreeHistory();
                        });
                    ...
                    const $btnClearHistory = $(
                        '<button class="lab-mini-btn btn-danger" id="labBtnClearSavedHistory" title="Dọn dẹp lịch sử Console và Tree DOM (Reset)">🧹 </button>'
                        );
                    ...
                    $btnClearHistory.on('click', function (e) {
                        ...
                        if (typeof window.__labAppendLog === 'function')
                            window.__labAppendLog(
                                '🧹 Đã dọn dẹp lịch sử Console và Tree DOM.',
                                'return');
                    });
                    ...
                    function v163UpdateFloatingConsole() {
                        const jsMax = $('#panelJs').hasClass(
                            'lab-panel-maximized');
                        const cssMax = $('#panelCss').hasClass(
                            'lab-panel-maximized');
                        const isConsoleHidden = $('#panelConsole').hasClass(
                            'lab-panel-hidden');

                        // [UPDATE] Tự động Căn Chỉnh Kích Thước khi ô JS/CSS phóng to
                        if ((jsMax || cssMax) && !isConsoleHidden) {
                            $('#panelJs .lab-sub-select').val('#panelConsole')
                                .trigger('change')
                            /*
                            $('#panelConsole')
                            ...
                            */
                            ...
                            $('#panelConsole').removeClass(
                                'lab-v163-floating-console');
                            $layoutEngine.removeClass(
                                'has-v163-floating-console');

                            $('.lab-panel.lab-panel-maximized').css('width',
                                '100%');
                        }
                        setTimeout(() => {
                            if (window.__labJsEditor) window
                    ...
                    window.__labV163UpdateFloatingConsole =
                        v163UpdateFloatingConsole;

                    $(document).on('click.v163FloatConsole',
                        '.lab-btn-max, #labBtnToggleOrientation, #labBtnResetLayout, #labFabBtn, .lab-btn-toggle',
                        function () {
                    ...
                    const $btnQuickExtract = $(
                        '<button class="lab-mini-btn btn-success" id="labBtnQuickExtractLinks" style="margin-right:4px;" title="Trích xuất toàn bộ Link bên trong Node này">🔗</button>'
                        );
                    $('#panelTreeDom .lab-panel-actions').prepend(
                        $btnQuickExtract);
                    ...
                    function v163SetCurrentTreeElement(element) {
                        ...
                        try {
                            localStorage.setItem(LS_TREE_TARGET_KEY, element
                                .outerHTML || '');
                        } catch (err) {}
                    }

                    $(document).on('contextmenu.v163CaptureTarget', function (
                    e) {
                        // [UPDATE] Chặn bắt sự kiện viền hắt vào bảng Sniffer
                        if ($(e.target).closest(
                                '#labHtmlSourceModal,#labMainDashboard, #labCssQuickMenu, .CodeMirror-hints, #panelSnifferLab,#quick-extract-modal,.lab-inspect-child'
                                ).length || $(e.target).is(
                                '#labSandboxIframe')) return;
                        v163SetCurrentTreeElement(e.target);
                    });

                    $('#labFamilyTreeBar').on('click.v163CaptureLayer',
                        '.lab-geo-btn',
                        function () {
                            setTimeout(() => {
                                const $pinned = $(
                                ...
                                if ($pinned.length)
                                    v163SetCurrentTreeElement(
                                        $pinned[0]);
                            }, 80);
                        });

                    // HÀM ĐƯỢC NÂNG CẤP: Thêm tham số customSelector để cố định vùng tìm kiếm dữ liệu
                    function v163ExtractLinksFromElement(rootElement,
                        extractType = 'default', customSelector = '') {
                        ...
                        const targetSelector = customSelector.trim() ||
                            'a[href]';
                        ...
                        if ($root.is(targetSelector) && $root.is('a[href]')) {
                            if (!links.includes(rootElement)) links.push(
                                rootElement);
                        }
                        ...
                        return links.map(a => {
                            ...
                            const attrTypes = ['title', 'alt',
                                'data-title', 'data-alt', 'src',
                                'name'
                            ];
                            ...
                            if (extractType === 'default') {
                                ...
                                name = $targetTag.length ? $targetTag
                                    .text() : $a.text();
                            }
                    ...
                    $btnQuickExtract.on('click', function (e) {
                        ...
                        const options = ['title', 'src', 'alt',
                            'data-title', 'data-alt', 'h1', 'h2',
                            'h3', 'span', 'p', 'b', 'i'
                        ];
    ...
    `);
                        ...
                        function processExtraction() {
                            let extractType = $typeInput.val().trim()
                                .toLowerCase();
                            ...
                            const customSelector = $selectorInput.val()
                                .trim();

                            let output = '';
                            if (v163CurrentTreeElement) {
                                output = v163ExtractLinksFromElement(
                                    v163CurrentTreeElement,
                                    extractType, customSelector);
                            }

                            if (!output) {
                                try {
                                    const savedOuter = localStorage
                                        .getItem(LS_TREE_TARGET_KEY);
                                    if (savedOuter) {
                                        const temp = document
                                            .createElement('div');
                                        temp.innerHTML = savedOuter;
                                        output =
                                            v163ExtractLinksFromElement(
                            ...
                            if (!output) output =
                                '[Quick Extract] Không tìm thấy dữ liệu phù hợp với cấu hình thiết lập!';

                            if (typeof window.__labAppendLog ===
                                'function') window.__labAppendLog(
                                output, 'return');
                            else $consoleLog.prepend(
                                '<div class="lab-log-item lab-log-return"><span>' +
                            ...
                            $popup.remove();
                            $('#panelJs .lab-sub-select').val(
                                '#panelConsole').trigger('change');
                        }

                        // --- BẮT SỰ KIỆN PHÍM TẮT TRÊN CẢ HAI Ô NHẬP ---
                        $typeInput.add($selectorInput).on('keydown',
                            function (evt) {
                                if (evt.key === 'Enter') {
                                    evt.preventDefault();
                                    processExtraction();
                        ...
                        $('#btn-extract-submit').on('click',
                            processExtraction);
                        $('#btn-extract-cancel').on('click',
                    function () {
                            $popup.remove();
                        });
                    });
                    ...
                    const v163LayerButtonMap = [
                        '#geoBtnTarget', '#geoBtnParent', '#geoBtnGrand',
                        '#geoBtnGreatGrand', '#geoBtnGreatGreatGrand',
                        '#geoBtnAncestors', '#geoBtnLayer7',
                        '#geoBtnLayer8', '#geoBtnLayer9', '#geoBtnLayer10'
                    ];
                    ...
                    function v163SetActiveLayerByDepth(depth) {
                        ...
                        $('.lab-geo-btn').removeClass('activeMode');
                        $((v163LayerButtonMap[v163ActiveDepth] ||
                            '#geoBtnTarget')).addClass('activeMode');
                    }

                    $(document).on('click.v163ActiveLayerFix',
                        '#labFamilyTreeBar .lab-geo-btn',
                        function (e) {
                            const btnId = this.id;
                            const directMap = {
                                geoBtnTarget: 0,
                            ...
                            if (btnId === 'geoBtnReverseDown') {
                                setTimeout(() => v163SetActiveLayerByDepth(
                                    Math.max(0, v163ActiveDepth - 1)
                                    ), 20);
                    ...
                    function v163SaveSnifferPosition() {
                        ...
                        try {
                            localStorage.setItem(LS_SNIFFER_POS_KEY, JSON
                                .stringify({
                                    top: rect.top,
                                    left: rect.left,
                                    width: rect.width,
                    ...
                    function v163RestoreSnifferPosition() {
                        ...
                        try {
                            ...
                            $snifferPanel.addClass('lab-v163-user-positioned')
                                .css({
                    ...
                    const oldSyncSnifferLayout = typeof syncSnifferLayout ===
                        'function' ? syncSnifferLayout : null;
                    if (oldSyncSnifferLayout && !window
                        .__labV163SyncSnifferWrapped) {
                        syncSnifferLayout = function () {
                            if (v163UserMovedSniffer && !$snifferPanel
                                .hasClass('lab-panel-hidden')) {
                                $snifferPanel.css('display', 'flex');
                                return;
                            }
                    ...
                    $snifferPanel.off('mousedown.v163Drag').on(
                        'mousedown.v163Drag', '.lab-panel-header',
                        function (e) {
                            if ($(e.target).closest(
                            ...
                            $snifferPanel.addClass(
                                'lab-v163-user-positioned').css({
                                left: startLeft + 'px',
                                top: startTop + 'px',
                                right: 'auto'
                            });

                            $(window).on('mousemove.v163SnifferDrag',
                                function (moveEvent) {
                                    $snifferPanel.css({
                            ...
                            $(window).on('mouseup.v163SnifferDrag',
                                function () {
                                    $(window).off(
                                        'mousemove.v163SnifferDrag mouseup.v163SnifferDrag'
                                        );
                                    v163UserMovedSniffer = true;
                    ...
                    $snifferPanel.off('mousedown.v163Resize').on(
                        'mousedown.v163Resize', '.lab-resizer',
                        function (e) {
                            e.preventDefault();
                            e.stopPropagation();
                            ...
                            $(window).on('mousemove.v163SnifferResize',
                                function (moveEvent) {
                                    $snifferPanel.css({
                                        width: Math.min(window
                                                .innerWidth -
                                                rect.left - 8,
                                                Math.max(240,
                                                    startW +
                            ...
                            $(window).on('mouseup.v163SnifferResize',
                                function () {
                                    $(window).off(
                                        'mousemove.v163SnifferResize mouseup.v163SnifferResize'
                                        );
                                    v163UserMovedSniffer = true;
                                    v163SaveSnifferPosition();
                                });
                    ...
                    function v163EscapeText(value) {
                        return $('<div>').text(value == null ? '' : String(
                            value)).html();
                    }
                    ...
                    function v163BuildSnifferGroupDomNode(title, linksMap,
                        color, defaultOpen) {
                        ...
                        const $label = $(
                            ...
                            ).text(`${title} (${linksMap.size})`);
                        const $children = $(
                            '<div class="tree-children" style="margin-left:18px;"></div>'
                            );
                        ...
                        if (linksMap.size === 0) {
                            $children.append(
                                '<div style="color:#777; font-style:italic; margin-left:14px;">(Trống)</div>'
                                );
                        } else {
                            linksMap.forEach((networkDetails, url) => {
                                const details = networkDetails || {};
                                const $item = $(
                                    '<div class="lab-sniffer-item" style="white-space:normal; margin-bottom:6px; padding:6px; background:rgba(255,255,255,0.03); border-left:2px solid #7f8c8d; cursor:pointer;"></div>'
                                    );
                                const $urlLine = $(
                                    '<div style="display:flex; gap:6px; align-items:flex-start;"></div>'
                                    );
                                const $urlSpan = $(
                                    '<span style="color:#e67e22; font-size:11px; word-break:break-all;"></span>'
                                    ).text(url);

                                const $detailsBox = $(
                                    '<div class="lab-sniffer-details"></div>'
                                    );
                                $detailsBox.html(
                                    v163BuildDetails('METHOD',
                                        details.method || 'UNKNOWN',
                                        false) +
                                    v163BuildDetails(
                                        'REQUEST HEADERS', details
                                        .headers ||
                                        'No Headers Captured', false
                                        ) +
                                    v163BuildDetails(
                    ...
                    window.__labRenderSnifferTree = function () {
                        ...
                        if (!$snifferBody.length || !window
                            .__labCapturedLinks) return;
                        $snifferBody.empty();
                        $snifferBody.append(v163BuildSnifferGroupDomNode(
                            '📁 MEDIA LINKS (.mp4, .m3u8, .ts)',
                            window.__labCapturedLinks.media,
                            '#2ecc71', true));
                        $snifferBody.append(v163BuildSnifferGroupDomNode(
                            '📁 EMBED RESOURCES (iframe, player)',
                            window.__labCapturedLinks.embed,
                            '#9b59b6', false));
                        $snifferBody.append(v163BuildSnifferGroupDomNode(
                            '📁 POST REQUESTS (Full Objects)',
                            window.__labCapturedLinks.post,
                            '#e74c3c', false));
                        $snifferBody.append(v163BuildSnifferGroupDomNode(
                            '📁 GET REQUESTS (Full Objects)', window
                            .__labCapturedLinks.get, '#3498db',
                            false));
                    };
        ...
        `;
            ...
            `;
                    ...
                    const $viewSourceBtn = $(
                        '<button class="lab-mini-btn" id="labBtnViewSource" title="Xem Cây DOM Gốc" style="margin-right:4px;">📄 Source Tree</button>'
                        );
                    ...
                    function createTreeDOM(node) {
                        if (node.nodeType === Node.COMMENT_NODE) {
                            ...
                            return $(
                                `<div class="lab-dom-comment">&lt;!-- ${escapeHTML(commentTxt)} --&gt;</div>`
                                )[0];
                        }
                        if (node.nodeType === Node.TEXT_NODE) {
                            ...
                            if (!text) return null;
                            return $(
                                `<div class="lab-dom-pure-text">${escapeHTML(text)}</div>`
                                )[0];
                        }
                        if (node.nodeType !== Node.ELEMENT_NODE) return null;

                        const tagName = node.tagName.toLowerCase();
                        if (['link', 'meta', 'br', 'hr', 'img', 'input']
                            .includes(tagName)) {
                            // Xử lý các thẻ tự đóng đơn giản inline
                            const $nodeWrap = $(
                                '<div class="lab-dom-node"></div>');
                            const $header = $(
                                '<div class="lab-dom-header"></div>');
                            $header.append(
                                '<span class="lab-dom-toggle empty">&nbsp;</span>'
                                );

                            const $openTag = $(
                                `<span class="lab-dom-tag-open">&lt;<span class="tag-name">${tagName}</span></span>`
                                );
                            if (node.attributes && node.attributes.length > 0) {
                                for (let attr of node.attributes) {
                                    $openTag.append(' ');
                                    $openTag.append($(
                                        `<span class="tag-attr" data-name="${attr.name}" data-val="${attr.value}"><span class="attr-name">${attr.name}</span>="<span class="attr-val">${attr.value}</span>"</span>`
                                        ));
                                }
                            }
                            $openTag.append(' /&gt;');
                        ...
                        const $toggle = $(
                            '<span class="lab-dom-toggle"></span>');
                        const complexNode = hasElementChildren || (
                            hasChildren && totalTextLength > 80);
                        ...
                        const $childrenContainer = $(
                            '<div class="lab-dom-children" style="display:none;"></div>'
                            );
                        const $blockClose = $(
                            `<div class="lab-dom-block-close" style="display:none; padding-left:16px;">&lt;/<span class="tag-name">${tagName}</span>&gt;</div>`
                            );

                        if (complexNode) {
                            $header.append(
                                '<span class="lab-dom-summary">...</span>');
                            $header.append($(
                                `<span class="lab-dom-tag-close" style="display:none;">&lt;/<span class="tag-name">${tagName}</span>&gt;</span>`
                                ));

                            for (let child of node.childNodes) {
                                const childDOM = createTreeDOM(child);
                                if (childDOM) $childrenContainer.append(
                                    childDOM);
                            }
                            $nodeWrap.append($header).append($childrenContainer)
                                .append($blockClose);
                        } else {
                            ...
                            if (txt) {
                                $header.append($(
                                    `<span class="lab-dom-text">${escapeHTML(txt)}</span>`
                                    ));
                            }
                            $header.append($(
                                `<span class="lab-dom-tag-close">&lt;/<span class="tag-name">${tagName}</span>&gt;</span>`
                                ));
                        ...
                        $header.on('click', function (e) {
                            if ($(e.target).closest(
                                    '.tag-name, .tag-attr, .lab-dom-text'
                                    ).length > 0) return;
                            if ($toggle.hasClass('has-children')) {
                                const isOpen = $childrenContainer.is(
                                    ':visible');
                                if (isOpen) {
                                    $childrenContainer.hide();
                                    $blockClose.hide();
                                    $header.find('.lab-dom-summary')
                                        .show();
                                    $toggle.text('▸');
                                } else {
                    ...
                    function executeFetchSource(targetUrl) {
                        $treeContainer.html(
                            '<div style="color:#aaa; padding:10px;">⌛ Đang phân tích và dựng bản đồ DOM Tree nguồn...</div>'
                            );

                        fetch(targetUrl)
                            .then(response => response.text())
                            .then(html => {
                                parsedHtmlDocument = new DOMParser()
                                    .parseFromString(html, 'text/html');
                                $treeContainer.empty();

                                // Dựng cây từ thẻ HTML gốc của trang được fetch
                                const treeRoot = createTreeDOM(
                                    parsedHtmlDocument.documentElement);
                                if (treeRoot) {
                                    $treeContainer.append(treeRoot);
                                    // Mở sẵn tầng đầu tiên cho thân thiện
                                    $(treeRoot).children('.lab-dom-header')
                                        .trigger('click');
                                }
                            ...
                            .catch(err => {
                                ...
                                $treeContainer.html(
                                    '<div style="color:#c0392b; padding:10px;">❌ Lỗi nạp nguồn mã: ' +
                                    err + '</div>');
                            });
                    }
                    ...
                    $urlFetchBtn.on('click', function (e) {
                        ...
                        if (!inputUrl) {
                            alert(
                                'Vui lòng nhập đường dẫn URL cần Fetch tiếp theo!');
                            return;
                        }
                    ...
                    $copySourceBtn.on('click', function (e) {
                        ...
                        if (!parsedHtmlDocument) {
                            alert(
                                'Không tìm thấy dữ liệu nguồn để sao chép!');
                            return;
                        }
                        ...
                        try {
                            const documentClone = parsedHtmlDocument
                                .cloneNode(true);
                            if (documentClone && documentClone
                                .documentElement) {
                                // 1. Quét sạch hoàn toàn mọi thẻ <script>
                                const allScriptTags = documentClone
                                    .querySelectorAll('script');
                                allScriptTags.forEach(script => script
                                    .remove());

                                // 2. Duyệt qua toàn bộ phần tử để xóa sự kiện inline và sửa URL tương đối
                                const allElements = documentClone
                                    .querySelectorAll('*');
                                ...
                                throw new Error(
                                    "Không thể clone phần tử gốc");
                            }
                        } catch (domError) {
                            console.warn(
                                "Tầng 1 (DOM) thất bại do dữ liệu quá nặng. Chuyển sang Tầng 2 (Regex)...",
                                domError);
                            ...
                            try {
                                ...
                                if (parsedHtmlDocument
                                    .documentElement) {
                                    rawHtml = parsedHtmlDocument
                                        .documentElement.outerHTML;
                                } else {
                                    rawHtml = parsedHtmlDocument.body
                                        .innerHTML;
                                }
                                // Xóa các thẻ script
                                ...
                                alert(
                                    '❌ Không thể xử lý mã nguồn của trang này bằng cả 2 phương pháp!');
                                return;
                            }
                        }
                        ...
                        if (finalCleanHTML) {
                            if (navigator.clipboard && navigator
                                ...
                                navigator.clipboard.writeText(
                                    ...
                                    .then(() => {
                                        showToastSuccess(
                                            "Đã sao chép Source Sạch!",
                                            e.clientX, e.clientY
                                            );
                                    })
                                    .catch(() => {
                                        executeFallbackCopyOrDownload
                                            (finalCleanHTML, e
                                                .clientX, e.clientY
                                                );
                                    });
                    ...
                    function executeFallbackCopyOrDownload(text, x, y) {
                        ...
                        try {
                            const $temp = $('<textarea>').val(text).appendTo(
                                'body').select();
                            copySuccess = document.execCommand('copy');
                            $temp.remove();
                            if (copySuccess) {
                                showToastSuccess("Đã sao chép (Dự phòng)!", x,
                                    y);
                                return;
                            }
                        } catch (err) {
                            console.error("Cách copy dự phòng cũng thất bại: ",
                                err);
                        }
                        ...
                        try {
                            ...
                            a.href = url;
                            a.download =
                                `clean_source_${new Date().getTime()}.html`;
                            ...
                            alert(
                                '⚠️ Do trang web này chặn quyền Sao chép, Script đã tự động chuyển hướng tải mã nguồn sạch về máy của bạn dưới dạng file .html để sử dụng hoàn hảo nhất!');
                        } catch (downloadError) {
                            alert(
                                '❌ Thất bại: Không thể sao chép và không thể tạo file tải về!');
                        }
                    }

                    // Hàm hiển thị thông báo Toast thành công
                    function showToastSuccess(msg, x, y) {
                        const $toast = $('<div class="lab-html-toast"></div>')
                            .text(msg);
                        $('body').append($toast);
                        $toast.css({
                            left: (x + 12) + 'px',
                    ...
                    $treeContainer.on('click',
                        '.tag-name, .tag-attr, .lab-dom-text, .lab-dom-pure-text',
                        function (e) {
                            e.stopPropagation();
                            let copyText = '';
                            ...
                            if ($(this).hasClass('tag-name')) {
                                const rawNode = $(this).closest(
                                    '.lab-dom-node').data('raw-node');
                                if (rawNode &&
                                    typeof loadElementToTreeMain ===
                                    'function') {
                                    loadElementToTreeMain(rawNode);
                                    $('#labFamilyTreeBar').css('display',
                                        'flex');
                                }
                            }

                            if (copyText && copyText.trim()) {
                                copyToClipboardWithToast(copyText.trim(), e
                                    .clientX, e.clientY);
                            }
                        });
                    ...
                    $(document).on('keydown.htmlSourceModal', function (e) {
                        if (e.key === 'Escape' && $modal.hasClass(
                                'lab-html-modal-active')) closeModal();
                    });
                    ...
                    function clearSearch() {
                        ...
                        $sidebarList.empty();
                        $treeContainer.find(
                            '.lab-tree-search-highlight, .lab-tree-search-current'
                            ).each(function () {
                    ...
                    function performSearch() {
                        ...
                        if (query === lastSearchQuery && searchMatches.length >
                            0) {
                        ...
                        $treeContainer.find(
                            '.tag-name, .attr-name, .attr-val, .lab-dom-text, .lab-dom-pure-text, .lab-dom-comment'
                            ).each(function () {
                            const $el = $(this);
                            const txt = $el.text();
                            const startIdx = txt.toLowerCase().indexOf(
                                lowerQuery);
                            ...
                            if (startIdx !== -1) {
                                ...
                                const regex = new RegExp('(' + query
                                    .replace(
                                        /[-\/\\^$*+?.()|[\]{}]/g,
                                        '\\$&') + ')', 'gi');
                                const highlightedHtml = safeTxt.replace(
                                    regex,
                                    '<span class="lab-tree-search-highlight">$1</span>'
                                    );
                                $el.html(highlightedHtml);

                                let displayTxt = txt.substring(startIdx,
                                    startIdx + 50);
                                if (txt.length > 50) displayTxt +=
                                '...';

                                const pTag = $el.closest(
                                        '.lab-dom-node').find(
                                        '.tag-name').first().text() ||
                                    'text';
                        ...
                        $searchCount.text(searchMatches.length > 0 ?
                            `1/${searchMatches.length}` : '0/0');
                        $sidebarList.html(listHtml);

                        $sidebarList.find('.lab-html-match-item').on('click',
                            function () {
                                jumpToMatch(parseInt($(this).attr(
                                    'data-idx'), 10));
                            });
                    ...
                    function jumpToMatch(index) {
                        ...
                        $targetElement.parents('.lab-dom-children').each(
                            function () {
                                const $childrenBlock = $(this);
                                if (!$childrenBlock.is(':visible')) {
                                    $childrenBlock.show();
                                    const $parentNode = $childrenBlock
                                        .closest('.lab-dom-node');
                                    const $pHeader = $parentNode.children(
                                        '.lab-dom-header');
                                    $pHeader.find('.lab-dom-toggle').text(
                                        '▾');
                                    $pHeader.find('.lab-dom-summary')
                                .hide();
                                    $parentNode.children(
                                        '.lab-dom-block-close').show();
                                }
                            });

                        // Cập nhật trạng thái Highlight cam đậm cho phần tử hiện tại đang chọn
                        $treeContainer.find('.lab-tree-search-current')
                            .removeClass('lab-tree-search-current').addClass(
                                'lab-tree-search-highlight');
                        const $localHighlight = $targetElement.find(
                            '.lab-tree-search-highlight').eq(0);
                        if ($localHighlight.length) {
                            $localHighlight.removeClass(
                                'lab-tree-search-highlight').addClass(
                                'lab-tree-search-current');
                        ...
                        $searchCount.text((index + 1) + '/' + searchMatches
                            .length);
                        $sidebarList.find('.lab-html-match-item').removeClass(
                            'active-match');
                        const $activeItem = $sidebarList.find(
                            `.lab-html-match-item[data-idx="${index}"]`);
                    ...
                    $searchPrev.on('click', function (e) {
                        e.stopPropagation();
                        if (searchMatches.length > 0) jumpToMatch(
                            currentMatchIndex - 1);
                    });
                    ...
                    function copyToClipboardWithToast(text, x, y) {
                        ...
                        const displayText = text.length > 35 ? text.substring(0,
                            32) + '...' : text;
                        const $toast = $('<div class="lab-html-toast"></div>')
                            .text('Đã sao chép: ' + displayText);
                        $('body').append($toast);
                        $toast.css({
                            left: (x + 12) + 'px',
                            top: (y + 12) + 'px'
                ...
                (function () {
                    ...
                    try {
                        ...
                        // --- Globals ---
                        const $cssInput = window.$ && $('#labCssInput').length ?
                            $('#labCssInput') : null;
                        const $jsInput = window.$ && $('#labJsInput').length ?
                            $('#labJsInput') : null;
                        ...
                        function selectorFromElement(el) {
                            ...
                            if (el.className && typeof el.className ===
                                'string') {
                                const cls = el.className.trim().split(/\s+/)
                                    .filter(Boolean).map(c => CSS.escape(c))
                                    .join('.');
                                if (cls) return '.' + cls;
                            }
                        ...
                        let shieldActive =
                        false; // Default OFF to avoid blocking on first install
                        ...
                        function shieldClickHandler(e) {
                            ...
                            const path = e.composedPath ? e.composedPath() : [e
                                .target
                            ];
                            for (let node of path) {
                                if (node instanceof Element && isInsideLabUI(
                                        node)) return;
                            }
                            e.preventDefault();
                            e.stopPropagation();
                        ...
                        function enableShield() {
                            ...
                            updateShieldBtn();
                            if (typeof window.__labAppendLog === 'function')
                                window.__labAppendLog('[Shield] ENABLED',
                                    'return');
                        }
                        ...
                        function disableShield() {
                            ...
                            try {
                                ...
                                window.__labAppendLog('Có lỗi xảy ra',
                                'return');
                            }
                            updateShieldBtn();
                            if (typeof window.__labAppendLog === 'function')
                                window.__labAppendLog('[Shield] DISABLED',
                                    'return');
                        }
                        ...
                        function createShieldBtn() {
                            if (!window.$) return;
                            $shieldBtn = $(
                                '<button class="lab-mini-btn" id="labBtnAntiHijack" style="margin-right:4px;" title="Bật/Tắt Anti-Hijack Shield">🛡️ OFF</button>'
                                );
                            const $target = $(
                                '#labRestoreBar .lab-restore-group').first();
                            if ($target.length) {
                                $target.append($shieldBtn);
                            } else {
                                // Fallback: append to dashboard header or body
                                const $header = $(
                                    '#labMainDashboard .lab-header');
                                if ($header.length) {
                        ...
                        function updateShieldBtn() {
                            ...
                            if (shieldActive) {
                                ...
                                }).text('🛡️ SHIELD ON').attr('title',
                                    'Anti-Hijack Shield đang BẬT — Click để tắt'
                                    );
                            } else {
                                $shieldBtn.css({
                                    background: '#27ae60',
                                ...
                                }).text('🛡️ SHIELD OFF').attr('title',
                                    'Anti-Hijack Shield đang TẮT — Click để bật'
                                    );
                            }
                        }
            ...
            `);
                            ...
                            $extractMenu.on('mousedown.labExtractDrag',
                                function (e) {
                                    // Chỉ kéo khi bấm vào vùng header hoặc border, không kéo khi bấm vào nút, row, pre
                                    if ($(e.target).closest(
                                            'button, .lab-extract-row, pre, code, input'
                                            ).length) return;
                                    e.preventDefault();
                                    e.stopPropagation();
                                    const rect = $extractMenu[0]
                                        .getBoundingClientRect();
                                    const startX = e.clientX,
                                        startY = e.clientY;
                                    const startLeft = rect.left,
                                        startTop = rect.top;
                                    $(window).on('mousemove.labExtractMove',
                                        function (moveEvent) {
                                            $extractMenu.css({
                                                left: (startLeft +
                                                        moveEvent
                                    ...
                                    $(window).on('mouseup.labExtractMove',
                                        function () {
                                            $(window).off(
                                                'mousemove.labExtractMove mouseup.labExtractMove'
                                                );
                                        });
                                });

                            $extractMenu.find('#labBtnExtractInline').on(
                                'click',
                                function (e) {
                                    e.stopPropagation();
                                    const css = getInlineStyleCss(
                                        _extractEl);
                                    if (css) addBlock('inline style', css);
                                    else if (typeof window
                                        .__labAppendLog === 'function')
                                        window.__labAppendLog(
                                            '⚠️ Không có inline style.',
                                            'log');
                                });
                            $extractMenu.find('#labBtnExtractSheets').on(
                                'click',
                                function (e) {
                                    e.stopPropagation();
                                    const rules =
                                        getStylesheetCssForSelector(
                                            _extractSelector);
                                    if (rules.length) {
                                        rules.forEach((r, i) => addBlock(
                                            'stylesheet #' + (i +
                                            1), r));
                                    } else {
                                        if (typeof window.__labAppendLog ===
                                            'function') window
                                            .__labAppendLog(
                                                '⚠️ Không tìm thấy rule stylesheet cho ' +
                                                _extractSelector, 'log');
                                    }
                                });
                            $extractMenu.find('#labBtnExtractComputed').on(
                                'click',
                                function (e) {
                                    e.stopPropagation();
                                    const css = getComputedCss(_extractEl);
                                    if (css) addBlock('computed style',
                                    css);
                                    else if (typeof window
                                        .__labAppendLog === 'function')
                                        window.__labAppendLog(
                                            '⚠️ Không có computed style khả dụng.',
                                            'log');
                                });
                            $extractMenu.find('#labBtnInjectToEditor').on(
                                'click',
                                function (e) {
                                    e.stopPropagation();
                                    const selected = _extractBlocks.filter((
                                            _, i) => $extractMenu.find(
                                            '.lab-extract-row').eq(i)
                                        .hasClass('selected'));
                                    const blocks = selected.length ?
                                        selected : _extractBlocks;
                                    if (!blocks.length) {
                                        alert(
                                            'Chưa có CSS nào. Hãy bấm Inline / Sheets / Computed trước.');
                                        return;
                                    }
                                    const merged = blocks.map(b => b
                                        .cssText).join('\n\n');
                                    const cur = ($cssInput && $cssInput
                                    .val() ? $cssInput.val() : '')
                                    .trim();
                                    const final = cur ? cur +
                                        '\n\n/* ===== CSS Extractor Injection ===== */\n' +
                                        merged : merged;
                                    if ($cssInput && $cssInput.length)
                                        $cssInput.val(final).trigger(
                                            'input');
                                    if (window.__labCssEditor && window
                                        .__labCssEditor.setValue) {
                                        window.__labCssEditor.setValue(
                                            final);
                                        if (window.__labCssEditor.setCursor)
                                            window.__labCssEditor.setCursor(
                                                window.__labCssEditor
                                                .lineCount(), 0);
                                    }
                                    if (typeof window.__labAppendLog ===
                                        'function') window.__labAppendLog(
                                        '🎨 Đã chèn ' + blocks.length +
                                        ' khối CSS vào CSS Editor.',
                                        'return');
                                    $extractMenu.hide();
                                });

                            $(document).on(
                                'click.closeCssExtractMenu contextmenu.closeCssExtractMenu',
                                function (e) {
                                    if (!$(e.target).closest(
                                            '#labCssExtractMenu').length)
                                        $extractMenu.hide();
                                });
                        }
                        ...
                        return '';
                            const body = el.style.cssText.split(';').filter(
                                    Boolean).map(s => '    ' + s.trim() + ';')
                                .join('\n');
                            ...
                            try {
                                for (let sheet of document.styleSheets) {
                                    ...
                                    for (let rule of rules) {
                                        ...
                                        const parts = selText.split(',').map(
                                            s => s.trim());
                                        let hit = false;
                                        for (let p of parts) {
                                            if (p.includes(selector) || p
                                                .includes(rawName)) {
                                                hit = true;
                                                break;
                                            }
                                ...
                                console.error(
                                    '[CSS Extractor] Stylesheet error:', e);
                            }
                            return results;
                        }
                        ...
                        function getComputedCss(el) {
                            ...
                            const props = ['display', 'visibility', 'position',
                                'top', 'left', 'right', 'bottom', 'width',
                                'height', 'margin', 'padding', 'background',
                        ...
                        function renderExtractList() {
                            ...
                            const $list = $extractMenu.find(
                                '#labCssExtractList');
                            $list.empty();
                            _extractBlocks = dedupBlocks(_extractBlocks);
                            if (!_extractBlocks.length) {
                                $list.html(
                                    '<div style="padding:8px 12px; color:#888;">Chưa có dữ liệu. Nhấn Inline / Sheets / Computed để trích xuất.</div>'
                                    );
                                return;
                            }
                ...
                `);
                        ...
                        function openExtractMenu(selector, x, y) {
                            ...
                            _extractBlocks = [];
                            $extractMenu.find('#labCssExtractTarget').text(
                                selector).attr('title', selector);
                            renderExtractList();
                            ...
                            if (py + h > window.innerHeight) py = window
                                .innerHeight - h - 12;
                            $extractMenu.css({
                        ...
                        document.addEventListener('click', function (e) {
                            ...
                            if (!t.matches('.html-val, .html-attr'))
                                return;
                            ...
                            if ($t.hasClass('html-val')) {
                                ...
                                const $prevAttr = $t.prevAll(
                                    '.html-attr').first();
                                if (!$prevAttr.length) return;
                                const attrName = $prevAttr.text().trim()
                                    .toLowerCase();
                                if (attrName === 'class') {
                                    const allClass = '.' + rawVal.split(
                                        /\s+/).filter(Boolean).join(
                                        '.');
                                    if (allClass !== '.')
                                        targetSelector = allClass;
                                } else if (attrName === 'id') {
                                    targetSelector = '#' + rawVal;
                                }
                            } else if ($t.hasClass('html-attr')) {
                                const attrName = $t.text().trim()
                                    .toLowerCase();
                                const $nextVal = $t.nextAll('.html-val')
                                    .first();
                                if (!$nextVal.length) return;
                                const rawVal = $nextVal.text().trim();
                                if (attrName === 'class') {
                                    const firstClass = rawVal.split(
                                        /\s+/).filter(Boolean)[0];
                                    if (firstClass) targetSelector =
                                        '.' + firstClass;
                                } else if (attrName === 'id') {
                            ...
                            if (targetSelector) {
                                openExtractMenu(targetSelector, e
                                    .clientX, e.clientY);
                                if (navigator.clipboard && navigator
                                    .clipboard.writeText) {
                                    navigator.clipboard.writeText(
                                ...
                                () => {});
                                    ...
                                    const $tmp = $('<textarea>').val(
                                        targetSelector).appendTo(
                                        'body').select();
                                    document.execCommand('copy');
                                    $tmp.remove();
                                }
                ...
                `;
                        ...
                        if (document.readyState === 'complete' || document
                        ...
                        if (localStorage.getItem(SHIELD_KEY) === 'true') {
                            setTimeout(function () {
                                if (localStorage.getItem(SHIELD_KEY) ===
                                    'true') enableShield();
                            }, 1000);
                        }
                        ...
                        console.error(
                            '[Enhancement Module v17.1] Initialization error:',
                            err);
                    }
                })();;
