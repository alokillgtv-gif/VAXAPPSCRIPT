...
window._$ = function (htmlOrBlock) {
    ...
    var instance = {
        ...
        find: function (selector) {
            ...
            if (selector.indexOf(":content(") !== -1) {
                var contentMatch = selector.match(
                    /:content\((?:"([^"]*)"|'([^']*)'|([^)]*))\)/);
                if (contentMatch) {
                    contentFilter = contentMatch[1] || contentMatch[2] || contentMatch[
                        3] || "";
            ...
            if (selectorToParse !== "") {
                ...
                if (idIndex !== -1) {
                    ...
                    selectorToParse = selectorToParse.substring(0, idIndex) + (
            ...
            for (var i = 0; i < this.elements.length; i++) {
                ...
                while ((pos = currentHtml.indexOf('<', pos)) !== -1) {
                    if (currentHtml.charAt(pos + 1) === '/' || currentHtml.charAt(pos +
                            1) === '!') {
                        pos++;
                        continue;
                    }
                    ...
                    var currentTagName = "";
                    if (spacePos === -1) {
                        currentTagName = fullOpenTag.substring(1, fullOpenTag.length -
                            1).toLowerCase();
                    } else {
                        currentTagName = fullOpenTag.substring(1, spacePos)
                        .toLowerCase();
                    }
                    var isMatched = true;
                    ...
                    if (isMatched && targetId) {
                        ...
                        if (idPos !== -1) {
                            ...
                            idMatchStr = fullOpenTag.substring(startQuote, fullOpenTag
                                .indexOf('"', startQuote));
                        } else {
                            idPos = fullOpenTag.indexOf("id='");
                            if (idPos !== -1) {
                                var startQuote = idPos + 4;
                                idMatchStr = fullOpenTag.substring(startQuote,
                                    fullOpenTag.indexOf("'", startQuote));
                            }
                        }
                    ...
                    if (isMatched && targetClasses.length > 0) {
                        ...
                        if (classPos !== -1) {
                            ...
                            classMatchStr = fullOpenTag.substring(startQuote,
                                fullOpenTag.indexOf('"', startQuote));
                        } else {
                    ...
                    if (isMatched && hasAttrFilter) {
                        ...
                        if (fullOpenTag.indexOf(searchStr1) === -1 && fullOpenTag
                            .indexOf(searchStr2) === -1) {
                            isMatched = false;
                        }
                    }
                    if (isMatched) {
                        ...
                        var selfClosingTags = ['img', 'source', 'input', 'br', 'hr',
                            'link', 'meta'
                        ];
                        if (selfClosingTags.indexOf(currentTagName) === -1 &&
                            ...
                            while (depth > 0 && scanPos < currentHtml.length) {
                                ...
                                if (nextOpen !== -1 && nextOpen < nextClose) {
                                    ...
                                    if (depth === 0) endTagPos = nextClose + closeStr
                                        .length;
                                }
                            }
                        }
                        ...
                        if (notSelector) {
                            ...
                            var hasNot = false;
                            if (isNotClass && fullOpenTag.indexOf('class="') !== -1 &&
                                fullOpenTag.indexOf(notValue) !== -1) hasNot = true;
                            if (isNotId && fullOpenTag.indexOf('id="') !== -1 &&
                                fullOpenTag.indexOf(notValue) !== -1) hasNot = true;
                ...
                if (isFirstFilter && subResults.length > 0) subResults = [subResults[
                0]];
                if (isLastFilter && subResults.length > 0) subResults = [subResults[
                    subResults.length - 1]];
                results = results.concat(subResults);
            }
            var newInstance = _$(results);
						Object.defineProperty(newInstance, 'sourceHtml', {
							value: this.sourceHtml || currentHtml,
							writable: true,
							enumerable: false, // Tiếp tục ẩn nó ở các instance mới tạo ra
							configurable: true
						});
						return newInstance;
        },
        ...
        next: function () {
            ...
            for (var i = 0; i < this.elements.length; i++) {
                ...
                if (nextOpen !== -1) {
                    ...
                    var fullOpenTag = this.sourceHtml.substring(nextOpen, endOpenTag +
                        1);
                    var spacePos = fullOpenTag.indexOf(' ');
                    var currentTagName = (spacePos === -1) ? fullOpenTag.substring(1,
                            fullOpenTag.length - 1).toLowerCase() : fullOpenTag
                        .substring(1, spacePos).toLowerCase();
                    var startTagPos = nextOpen;
                    var endTagPos = endOpenTag + 1;
                    var selfClosingTags = ['img', 'source', 'input', 'br', 'hr', 'link',
                        'meta'
                    ];
                    if (selfClosingTags.indexOf(currentTagName) === -1 && fullOpenTag
                        .indexOf('/>') === -1) {
                        var depth = 1;
        ...
        parent: function () {
            ...
            for (var i = 0; i < this.elements.length; i++) {
                ...
                while (scanPos >= 0) {
                    ...
                    if (this.sourceHtml.charAt(openTagPos + 1) !== '/' && this
                        .sourceHtml.charAt(openTagPos + 1) !== '!') {
                        var endOpenTag = this.sourceHtml.indexOf('>', openTagPos);
                        if (endOpenTag !== -1 && endOpenTag > openTagPos) {
                            var fullOpenTag = this.sourceHtml.substring(openTagPos,
                                endOpenTag + 1);
                            var spacePos = fullOpenTag.indexOf(' ');
                            var currentTagName = (spacePos === -1) ? fullOpenTag
                                .substring(1, fullOpenTag.length - 1).toLowerCase() :
                            ...
                            if (selfClosingTags.indexOf(currentTagName) === -1 &&
                                ...
                                while (depth > 0 && sPos < this.sourceHtml.length) {
                                    var nOpen = this.sourceHtml.indexOf(openStr, sPos);
                                    var nClose = this.sourceHtml.indexOf(closeStr,
                                    sPos);
                                    if (nClose === -1) break;
                                    if (nOpen !== -1 && nOpen < nClose) {
                                        depth++;
                                        ...
                                        depth--;
                                        sPos = nClose + closeStr.length;
                                        if (depth === 0) endTagPos = nClose + closeStr
                                            .length;
                                    }
                                }
                            }
                            if (endTagPos >= idx + elem.length) {
                                var parentBlock = this.sourceHtml.substring(openTagPos,
                                    endTagPos);
                                if (results.indexOf(parentBlock) === -1) results.push(
                                    parentBlock);
                                break;
                            }
                        }
        ...
        closest: function (selector) {
            ...
            for (var i = 0; i < this.elements.length; i++) {
                ...
                while (parentObj.elements.length > 0) {
                    ...
                    if (isMatched) {
                        if (results.indexOf(parentElem) === -1) results.push(
                        parentElem);
                        break;
                    }
		...
		// Định nghĩa ẩn sourceHtml
		Object.defineProperty(instance, 'sourceHtml', {
			value: typeof htmlOrBlock === 'string' ? htmlOrBlock : '',
			writable: true,
			enumerable: false, // Ẩn khỏi console và các vòng lặp duyệt key
			configurable: true
		});
    return instance;
};